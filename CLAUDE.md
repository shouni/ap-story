...
**This app owns the default style suffixes, but no default model names.** go-comic-kit keeps no defaults for either and rejects an empty one with `ports.ErrConfigInvalid`. Style suffixes are per-work art direction, so `config.Default*` supplies them (`AIConfig.applyDefaults` during `normalize`). Model IDs rot on Google's release schedule, so nobody defaults them: `GEMINI_MODELS` and `IMAGE_MODELS` are comma-separated lists whose head is the model used, and `ValidateEssentialConfig` fails startup when one is missing — `IMAGE_MODELS` for every role, `GEMINI_MODELS` only for `worker`, since the web side never generates a script. Concurrency, timeouts and the chapter/panel limits are the opposite case and keep their kit-side defaults, so leave those zero unless the deployment overrides them.
