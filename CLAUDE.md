...
**No art direction lives in config at all — only model names, and those have no defaults.** Art direction is per *work*, not per deployment, so every style word lives in `assets/prompts/styles.json` and is chosen per job (see the style-mode section below). `STYLE_SUFFIX` and `DESIGN_STYLE_SUFFIX` are gone, and so are the kit's `Config.StyleSuffix` / `Config.DesignStyleSuffix` — a knob that only relayed a string nobody read is worse than no knob. Model IDs rot on Google's release schedule, so nobody defaults them: `GEMINI_MODELS` and `IMAGE_MODELS` are comma-separated lists whose head is the model used, and `ValidateEssentialConfig` fails startup when one is missing — `IMAGE_MODELS` for every role, `GEMINI_MODELS` only for `worker`, since the web side never generates a script. Concurrency, timeouts and the chapter/panel limits are the opposite case and keep their kit-side defaults, so leave those zero unless the deployment overrides them.
