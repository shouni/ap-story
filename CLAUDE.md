...
**This app owns the default model names and style suffixes** (`config.Default*`, applied by `AIConfig.applyDefaults` during `normalize`). go-comic-kit deliberately keeps no defaults for those five fields and rejects an empty one with `ports.ErrConfigInvalid`, because model IDs rot on Google's release schedule and style suffixes are per-work art direction — neither should need a kit release to change. Concurrency, timeouts and the chapter/panel limits are the opposite case and keep their kit-side defaults, so leave those zero unless the deployment overrides them.
